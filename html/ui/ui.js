		...
		if (response.ok) {
			...
			files.forEach(file => {
				...
				if (file.uuid) {
					...
					if (file.position) {
						try {
							globalData["filePositionsByUUID"][file.uuid] = file.position;
						} catch {}
					}
				}
			});
...
async function saveFilePositions(specificFiles = null) {
	try {
		...
		await post("files/save-positions", {
	...
	try {
		if (globalData["mTab"][key] != true) {
			if (globalData["windows"][kind] && (key == null)) {
				globalData["windows"][kind]["size"] = {
					width,
					height
				};
			}
			if (globalData["openedWindows"][key]) {
				globalData["openedWindows"][key]["size"] = {
					width,
					height
				};
				...
				if (file) {
					...
					if (fileUuid) {
						window.localStorage.setItem(`filesize_${fileUuid}`, JSON.stringify({
							width: width,
							height: height
						}));
					}
...
function setWindowPosition(kind, key, x, y) {
	try {
		if (globalData["mTab"][key] != true) {
			if (globalData["windows"][kind] && (key == null)) {
				globalData["windows"][kind]["position"] = {
					x,
					y
				};
			}
			if (globalData["openedWindows"][key]) {
				globalData["openedWindows"][key]["position"] = {
					x,
					y
				};
				const file = globalData["openedWindows"][key].file;
				if (file) {
					const fileUuid = (globalData["fileUuids"] && globalData["fileUuids"][file]) ? globalData["fileUuids"][file] : null;
					if (fileUuid) {
						window.localStorage.setItem(`filepos_${fileUuid}`, JSON.stringify({
							x: x,
							y: y
						}));
						globalData["filePositionsByUUID"][fileUuid] = {
							x: x,
							y: y
						};
					}
	...
	const onMouseUp = function(e) {
		...
		if (hasMoved) {
				...
				if (moveCandidates.length > 0) {
					...
					(async () => {
						for (const df of moveCandidates) {
							...
							const resp = await post('files/move', {
								old: orig,
								new: newPath
							});
							try {
						...
						try {
							document.querySelectorAll('.folder-window').forEach(win => {
								...
								if (p === folderName) {
									win.dispatchEvent(new CustomEvent('folder:refresh', {
										bubbles: true,
										detail: {
											path: folderName
										}
			...
			if (inWin) {
				...
				if (folderWin) {
					...
					(async () => {
						...
						if (toRemoveFromDesktop.length) await saveFilePositions([]);
						try {
							folderWin.dispatchEvent(new CustomEvent('folder:refresh', {
								bubbles: true,
								detail: {
									path: targetPath
								}
			...
			draggedFiles.forEach(df => {
				...
				if (globalData["filePositions"][fileName].col === finalPos.col && globalData["filePositions"][fileName].row === finalPos.row) {
					return;
				}
				globalData["filePositions"][fileName] = {
					col: finalPos.col,
					row: finalPos.row
				};
...
function loadFiles(files) {
	...
	for (let i = 0; i < items.length; i++) {
		const d = items[i],
			...
			ic = globalData["extensions"][d.type];
...
document.addEventListener("contextmenu", (event) => {
	...
	if (target) {
		...
		if (isMultiSelected) {
			try {
				for (const fname of globalData["selectedFiles"]) {
					...
					if (el && el.getAttribute('extension') === 'Folder') {
		...
		if (isDesktop) {
			...
			globalData["contextClickPosition"] = {
				x: clickX,
				y: clickY
			};
		} else {
...
async function handleFiles() {
	...
	if (f.size > window.maxUploadSize) {
		if (window.notify) window.notify('File too large. Maximum size: ' + (window.maxUploadSize / (1024 * 1024)).toFixed(0) + ' MB', {
			type: 'error'
		});
		this.value = '';
		return;
	...
	try {
		...
		response = '';
		if (window.notify) window.notify('Upload failed', {
			type: 'error'
		});
	} finally {
		hideBusyOverlay();
	...
	if (response != "Updated") {
		loadFiles("[" + response + "]");
		if (window.notify) window.notify('Upload completed', {
			type: 'success'
		});
	}
};
...
async function Fopen(file, extension, key, options) {
	...
	if (extension == "Text document") {
		...
		const text = await get(url);
		openCodeEditor(text, fileName, key, {
			readOnly: !!isFromArchive,
			sourceUrl: looksArchiveUrl ? file : (isFromArchive ? url : undefined)
		});
...
async function createNewItem(type, defaultExt = '') {
  ...
  `;
	...
	const commit = async (cancel) => {
		...
		let resp;
		if (type === 'file') {
			resp = await post('files/upload', {
				file: newName,
				data: ''
			});
		} else {
			resp = await post('files/folders/create', {
				path: newName
			});
		}
...
async function Fremove(file) {
	...
	for (const fname of filesToRemove) {
		const response = await post("files/remove", {
			"file": fname
		});
		if (response == "Removed" || response == "File not found") {
			...
			if (typeof fname === 'string' && fname.endsWith('/')) {
				try {
					closeFolderWindowsForPath(fname);
				} catch {}
			}
		}
	}
...
async function Fpaste() {
	...
	try {
		...
		await Frefresh();
		if (targetWin) {
			const event = new CustomEvent('folder:refresh', {
				detail: {
					path: destPath
				}
			});
...
async function createArchive(files) {
	...
	try {
		const response = await post("files/archive/create", {
			files: files
		});
	...
	try {
		...
		const commit = async (cancel) => {
			...
			try {
				...
				if (j && j.error && !j.error.includes("move a folder")) {
					restore();
					prompt(j.error, 'Rename', 'Ok');
					return;
				}
			} catch {}
			finished = true;
			...
			e.dispatchEvent(new CustomEvent('folder:refresh', {
				bubbles: true
			}));
			await Frefresh();
		};
...
async function FremoveInPath(fullPath, key) {
	try {
		hideContext();
		const resp = await post('files/remove', {
			file: fullPath
		});
		try {
			const j = JSON.parse(resp);
			if (j && j.error) {
				await prompt(j.error, 'Remove', 'Ok');
				return;
			}
		} catch {}
		if (typeof fullPath === 'string' && fullPath.endsWith('/')) {
			try {
				closeFolderWindowsForPath(fullPath);
			} catch {}
		}
		await Frefresh();
		const e = document.getElementById("folder-win-" + key);
		if (e) e.dispatchEvent(new CustomEvent('folder:refresh', {
			bubbles: true
		}));
		return resp;
	} catch (e) {}
...
async function saveNotepad(file) {
	if (document.getElementById(file + "Text").value == globalData[file]) {
		return
	};

	var response = await post("files/upload", {
		"file": file,
		"data": document.getElementById(file + "Text").value
	});
  ...
  `;
	...
	var win = new WinBox({
		...
		onresize: (width, height) => {
			setTimeout(() => {
				setWindowSize("notepad", key, width, height);
			}, 100);
		},
		onmove: (x, y) => {
			setTimeout(() => {
				setWindowPosition("notepad", key, x, y);
			}, 150);
		},
		onclose: (force) => {
			if (force) {
...
function getAceModeForFilename(filename) {
	...
	const map = {
		...
		cbl: 'ace/mode/cobol',
		f: 'ace/mode/fortran',
		f90: 'ace/mode/fortran',
		f95: 'ace/mode/fortran',
		...
		njk: 'ace/mode/nunjucks',
		jinja: 'ace/mode/django',
		jinja2: 'ace/mode/django',
...
async function openCodeEditor(text, file, key, opts) {
	...
	const win = new WinBox({
		...
		onresize: (width, height) => {
			setTimeout(() => {
				setWindowSize('code', key, width, height);
			}, 100);
		},
		onmove: (x, y) => {
			setTimeout(() => {
				setWindowPosition('code', key, x, y);
			}, 150);
		},
		onclose: (force) => {
			if (force) {
...
async function openArchive(file, key) {
	...
	function renderTable() {
		...
		sorted.forEach(f => {
			if (f && f.name) pathToIsDir.set(f.name, !!f.is_dir);
		});
		...
		e.querySelectorAll('.archive-table tbody tr').forEach(row => {
			row.addEventListener('click', (ev) => {
				...
				if (!p || p === '..') return;
				if (ev.ctrlKey) {
					if (selectedRows.has(p)) selectedRows.delete(p);
					else selectedRows.add(p);
				} else {
					selectedRows.clear();
					selectedRows.add(p);
				}
			...
			row.addEventListener('mousedown', (downEvent) => {
				...
				const ensureFolderTree = async (targetPath) => {
					...
					for (let i = 0; i < parts.length; i++) {
						...
						try {
							await post('files/folders/create', {
								path: acc + '/'
							});
						} catch {}
					}
				...
				const extractToDesktopAt = async (clientX, clientY) => {
					try {
						...
						for (const item of draggedItems) {
							...
							if (item.isDir) {
								try {
									const resp = await post('files/archive/extract', {
										archive: file,
										path: itemPath,
										dest: baseName
									});
									...
									try {
										const j = JSON.parse(resp);
										if (j && j.file) finalFolder = j.file.endsWith('/') ? j.file : (j.file + '/');
									} catch {}
								...
								const uploadFile = new File([blob], baseName, {
		...
		if (content) {
			...
			const onMouseMove = (mv) => {
				...
				const x1v = Math.min(startX, cx);
				const y1v = Math.min(startY, cy);
				const x2v = Math.max(startX, cx);
				const y2v = Math.max(startY, cy);
	...
	const win = new WinBox({
		...
		onresize: (width, height) => {
			setTimeout(() => {
				setWindowSize('archive', key, width, height);
			}, 100);
		},
		onmove: (x, y) => {
			setTimeout(() => {
				setWindowPosition('archive', key, x, y);
			}, 100);
		},
		onclose: (force) => {
			delete globalData["openedWindows"][key];
    ...
    `;
		...
		e.querySelectorAll('.archive-table tbody tr').forEach(row => {
			...
			row.addEventListener('mousedown', (downEvent) => {
				...
				const onMouseMove = (mv) => {
					const dx = Math.abs(mv.clientX - sx),
						dy = Math.abs(mv.clientY - sy);
				...
				const onMouseUp = async (up) => {
					...
					if (dragging) {
						...
						if (targetRow && targetRow.dataset.path && targetRow.dataset.path !== '..') {
							...
							const filesContainer = document.getElementById('Files');
							const containerRect = filesContainer.getBoundingClientRect();
							if (up.clientX < containerRect.left || up.clientX > containerRect.right || up.clientY < containerRect.top || up.clientY > containerRect.bottom) {} else if (elAt && elAt.closest('.winbox')) {} else {
...
function openImage(file, key, title) {
  ...
  `;
	...
	const win = new WinBox({
		...
		onresize: (width, height) => {
			setTimeout(() => {
				setWindowSize('image', key, width, height);
			}, 100);
		},
		onmove: (x, y) => {
			setTimeout(() => {
...
async function openVideo(file, file_type, key, title) {
	...
	new WinBox({
		...
		onresize: (width, height) => {
			setTimeout(() => {
				setWindowSize(file_type, key, width, height);
			}, 100);
		},
		onmove: (x, y) => {
			setTimeout(() => {
				setWindowPosition(file_type, key, x, y);
			}, 100);
		},
		...
		oncreate: () => {
			globalData["openedWindows"][key] = {
				"file": file,
				"extension": file_type.charAt(0).toUpperCase() + file_type.slice(1),
				"size": {
					"width": windowData.width,
					"height": windowData.height
				},
	...
	if (e.key === 'Delete') {
		...
		if (focusedWin) {
			...
			if (folderWin && folderWin.selectedRows && folderWin.selectedRows.size > 0) {
				...
				for (const s of sel) {
					await FremoveInPath(s, winKey);
				}
				const event = new CustomEvent('folder:refresh', {
					detail: {
						path: folderWin.dataset.path
					}
				});
